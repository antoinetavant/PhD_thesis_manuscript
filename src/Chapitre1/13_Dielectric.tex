...
\section{Dielectrics boundary condition}
  ...
  \Cref{fig-2D} illustrate the configuration in the radial-azimuthal plan highliting the radial boundary conditions.
  ...
  \begin{equation} \label{eq-gauss}
    \partial_r \phi = \frac{\sigma}{\epsilon_0}
  \end{equation}
  ...
  \subsection{Non-uniform mesh}
    ...
    In the dielectric layers, there is no particles nor charges.
    Hence, the numerical constraints on the cell size are not applicable, and the cell size can be increased.
    In order to reduce the cell size difference between two neighbouring cells, we uses an exponential growth of the cell size in the radial direction.
  ...
  The dielectric permittivity is $\epsilon= \epsilon_r \epsilon_0$ with $\epsilon_r$ the relative permittivity of the dielectric.
  ...
  The cell $(i,j)$ has four direct neighbours:
  \begin{itemize}
  ...
  \subsection{Poisson equation discretization}
    ...
    This means that the permittivity $\epsilon = \epsilon_0 \epsilon_r$ is  constant over a cell.
    In order to discretize the Poisson equation, we integrate \vref{eq-poisson} over the cell volume
    ...
    We can writ the electric flux as
  ...
  \subsection{Verifications}
    ...
    The relative permittivity of the dielectric inside the capacitor is set to $\epsilon_r = 8$, and a surface charge of  $\sigma = 8$~nC.cm$^{-2}$ is imposed on one side, and $-8$~nC.cm$^{-2}$ on the other side.
    The expected electric field in the capacitor using the infinite plans approximation is $E = \sigma/(\epsilon_0\epsilon_r) = 1.15$~kV.mm$^{-1}$.
  ...
  \subsection{Interface at the cell centre}
    ...
    The decomposition is the same as previously, except for the permittivity that can have two different values: one in the North half-plan $\epsr{i,j}^n$ and another in the South half plan $\epsr{i,j}^s$.
    ...
    The discretisation of the Poisson \cref{eq-poisson} is follows the same path as previously.
  ...
  \subsection{Surface charges for centred interface}
    ...
    However, it better conserves the plasma domain when the dielectric layer is not modeled and that Dirichlet conditions are applied.
