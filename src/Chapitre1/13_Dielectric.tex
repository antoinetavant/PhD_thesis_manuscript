...
\section{Dielectrics boundary condition}
  ...
  \subsection{Verifications}
    ...
    \begin{figure}[hbtp]
      ...
      \label{fig-surface}
    \end{figure}
    \inlinenote{Improve the figure: Labels, theoretical value, legend, etc. Maybe Zoom at the boundary ?}
  ...
  \subsection{Interface at the cell center}
    ...
    The discretization of the Poisson \cref{eq-poissonsum} is follows the same path as previously, except that the electric flux in not constant anymore so that \cref{eq-poissonsum} becomes
    \begin{equation}
    \oint_{\C} (-\epsilon \grad \phi) \cdot \vect{n} dS = \sum_{k\in(E,W,N,S)} S^k_{i,j} <\vect{D}^k_{i,j} \cdot \vect{n}>.
    \end{equation}
