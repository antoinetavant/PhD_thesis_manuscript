...
\section{Dielectrics boundary condition}
  ...
  \subsection{Poisson equation discretization}
    ...
    \inlinenote{Introduce what is symmetric system}
  ...
  \subsection{Interface at the cell centre}
    ...
    The discretisation of the Poisson \cref{eq-poissonsum} is follows the same path as previously,except that the electric flux in equation in not constant any more, so that \cref{eq-poissonsum} becomes
    \begin{equation}
    \oint_{\C} (-\epsilon \grad \phi) \cdot \vect{n} dS = \sum_{k\in(E,W,N,S)} S^k_{i,j} <\vect{D}^k_{i,j} \cdot \vect{n}>.
    \end{equation}
