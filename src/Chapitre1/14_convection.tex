...
\section{Axial Convection model}
  ...
  \inlinenote{In all this section: get $\theta$ back to $y$ instead of $x$}
  ...
  \subsection{Effect of the noise on the electric field}
    ...
    \subsection{Effects on a \ac{2D} simulation domain}
      ...
      \Cref{fig-dftLr} shows the \ac{DFT} of the source term $\rho_1$, the resulting azimuthal electric field $\aziE_{, 1}$ and plasma potential $\phi_1$ computed on the centerline of the simulation domain, for three different radial lengths expressed in number of cell in the radial direction $N_x$.
      ...
      \begin{figure}[hbtp]
        ...
        \caption{\ac{DFT} of the source term $\rho_1 = \N(0, \stdconv)$, the resulting azimuthal electric field $\aziE_{, 1}$ and plasma potential $\phi$ computed on the centerline of a radial-azimuthal simulation, and the Equivalent \ac{1D} source term defined by \cref{eq-equirho}. The azimuthal length is $L_{\theta}=50$ cm. The \ac{DFT} are averaged 200 times  }
        \label{fig-dftLr}
