...
\section{Axial convection of the particles}
  ...
  \subsection{Numerical noise of Lafleur's convection model}
    ...
    Seen by the charge density, this is similar to Poisson noise, also named shot noise, on the charge density.\footnote{Actually, this noise is the combination of Poisson noise with a uniform noise as we use a cloud in cell method. In addition it is happening twice for every particle convection, once with a positive charge and once with a negative charge. But the result is not affected by this simplification due to the central limit theorem.}
  ...
  \subsection{Effect of the noise on the electric field}
    ...
    \subsection{Noiseless convection model}
      ...
      In previous section, we have shown that the convection model induces a noise in the charge density, that produces an azimuthal electric field which amplitude depends on the azimuthal length.
