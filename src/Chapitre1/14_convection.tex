...
\section{Axial Convection model}
  ...
  \subsection{Numerical artefacts}
    ...
    However, the value of this upper limit depends on the axial length, such that the if the axial length decreases, the upper limit of the azimuthal length decreases as well.
  ...
  \subsection{Effect of the noise on the electric field}
    ...
    \subsection{Noiseless convection model}
      ...
      This model is used in \cref{ch-2,ch-4}. 
      ...
      Hence, in \cref{ch-5} we uses the modified convection model.
    ...
    \subsection{Effects on a \ac{2D} simulation domain}
      ...
      \Cref{fig-dftLr} shows the \ac{DFT} of the source term \[\rho_1 = \N(0, \stdconv),\] the resulting azimuthal electric field $\aziE_{, 1}$ and plasma potential $\phi$ computed on the centreline of the simulation domain in the radial direction, for three different radial lengths expressed in number of cell $N_x$.
      ...
      \end{equation}
      The results are given for different radial lengths $N_x=15,50 \text{ and } 200$, while the azimuthally length $N_y=200$ is kept constant.
      ...
      \begin{figure}[hbtp]
        ...
        \caption{\ac{DFT} of the source term $\rho_1 = \N(0, \stdconv)$, the resulting azimuthal electric field $\aziE_{, 1}$ and plasma potential $\phi$ computed on the centreline of a radial-azimuthal simulation, and the Equivalent \ac{1D} source term defined by \cref{eq-equirho}. The azimuthal length is $L_{\theta}=50$ cm. }
        \label{fig-dftLr}
