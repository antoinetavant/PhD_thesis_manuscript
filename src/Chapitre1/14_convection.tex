...
\section{Axial Convection model}
  ...
  \subsection{Numerical artefacts}
    ...
    We can see that for a given value of the axial length, the azimuthal must be less than a specific value to present physical results.
  ...
  \subsection{Numerical noise of Lafleur's convection model}
    ...
    Seen by the charge density, this is similar to Poisson noise, also named shot noise, on the charge density.\footnote{Actually, this noise is the combination of Poisson noise with a uniform noise, happening twice for every particles convection, once with a positive charge and once with a negative charge.}
