  ...
  \subsection{Effect of the noise on the electric field}
    ...
    \subsection{Effects on a \ac{2D} simulation domain}
      ...
      \begin{figure}[hbtp]
        ...
        \caption{\ac{DFT} of the source term $\rho_1 = \N(0, \stdconv)$, the resulting azimuthal electric field $\aziE_{, 1}$ and plasma potential $\phi$ computed on the centreline of a radial-azimuthal simulation, and the "equivalent" source term defined by \cref{eq-equirho}. $N_y=200$. }
        \label{fig-dftLr}
        
      \end{figure}
      
      \inlinenote{Enlever $h$ de term. Peu-etre fair une seul legende}
