...
The \ac{SEE} rate (or yiel$\rate$ is the emission probability $\proba$ averaged over the electron flux at the wall.
...
\begin{figure}[hbt]
  ...
  \end{tabular}
  \caption{({\bf a}) Plasma potential drop to the wall $\dphi$ normalized by the electron bulk temperature $\Te$ as a function of the electron rate (blue) measured in the \ac{PIC} simulations and (orange) calculated with   \cref{eq-dphi2}; ({\bf b})  the \ac{SEE} rate $\rate$ (blue) measured in the PIC simulation, and (orange)  calculated with \cref{eq-rate2}}
