...
The \ac{2D} simulation plane used so far in this work was the radial-azimuthal plane, so that the plasma-wall interaction could be modeled while self-consistently simulating the azimuthal instability responsible for the axial electron transport.
In this configuration, we implemented a convection algorithm in order to model a part of the axial convection.
However, there are other missing mechanism of the axial direction in this simulation.
In particular\string: the axial electric field is imposed; the wave is not convected; the impact of the axial gradients, such as the diamagnetic drift, are not included; the ionization region is not modeled.

The axial-azimuthal \ac{2D} simulation domain is used to study these mechanism.
Numerous \ac{2D} \ac{PIC} simulation studies exists that model the axial-azimuthal plane \citep{adam2004,coche2014,boeuf2018,taccogna2019}.
However, in this model the impact of the radial direction is now missing.
Therefore, we propose a first step to model the radial direction in the axial-azimuthal simulation.

After presenting the simulation domain and characteristics in \cref{subsec-ztheta_description,subsec-breathmod,subsec-fakeR}, we compare the impact of the radial losses modeled on the simulation results, conserning the discharge characteristics in \cref{sec-Zthetaresults}, then focusing on the azimuthal instability in \cref{sec-Zthetaresults}.


\subsection{Description of the simulation domain} \label{subsec-ztheta_description}
To ease the understanding of the different simulations, \Cref{fig-3Dschematic} shows the simplified \ac{HET} annular channel with both the radial-azimuthal and the axial-azimuthal \ztheta domain.
...
\end{enumerate} 
These two cases are significantly different in the physics modeled and the behavior observed, as Boeuf's test-case will converge toward a steady-state, in contrast to Coche's test-case.
...
As the neutralization of the beam is modeled, the current injected is equal to the anode current:
...
Thus, the plasma potential is corrected so that the potential is $\phi=0$ at the location of injection.
...
The electrons are partially reflected, such that results in an electron flux absorbed at the wall equal to the ion flux.
...
In the results given in the next sections are obtained with modeling the radial losses in the totality of the simulation domain.
We choose to do so in order not to introduce a discontinuity in the boundary condition, as well as to increase the impact of the radial losses on the simulation results.
