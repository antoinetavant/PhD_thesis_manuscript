...
\Cref{fig-cut2D} shows the temporal evolution of the azimuthal electric field $E_{\theta}$ when no radial losses are modeled at the positions $z_u$ and $z_d$.
...
\Cref{fig-fft2D_noLr_zu}.{\bf a} shows the \ac{2D} \ac{FFT} in the upstream region.
...
\Cref{fig-fft2D_Lr4_zd,fig-fft2D_Lr2_zd} show the \ac{2D} \ac{FFT} obtained with the radial losses in the two regions region.
...
The impact of the radial losses on the wave can be better seen in \Cref{fig-boeuf_fft_comparasion}, where the \ac{1D} spectra obtained in the downstream region for the three cases are compared.
...
The test-case proposed by Boeuf allows the simulation to reach a steady-state.
...
\Cref{fig-ion-trap_temp_Lr} shows the same information as \cref{fig-ion-trap_temp_noLr} but with the radial losses modeled.
...
\Cref{fig-ionwavetrapping_axial} shows the axial profile of the ion-wave trapping averaged in time between $t=6\,\micro\second$ and $t=10\,\micro\second$ for the three cases.
