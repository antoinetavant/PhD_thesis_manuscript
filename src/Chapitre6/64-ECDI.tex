...
\Cref{fig-rfluxs} shows the azimuthal-axial distribution of (left) the radial flux measured in the \ac{PIC} simulation, and (right) the electron density, at $L_R=2\,\centi\meter$ at $t=10\,\micro\second$ for the case $L_R=2\,\centi\meter$.
...
To give a better understanding of the wave observed, \Cref{fig-cut2D} shows the temporal evolution of the azimuthal electric field $E_{\theta}$ when no radial losses are modeled at the positions $z_u$ and $z_d$.
...
The evolution of the waves can be better seen in \Cref{fig-boeuf_fft_comparasion}, where the \ac{1D} spectra obtained in the downstream region for the three cases are overlaid.
...
The low frequency oscillations looks like a non-linear saturation. Have have seen that because of the reduction of $u_{, \theta}$ and the present of $\gamma_R$, the effective growth rate is reduced by the radial losses.
