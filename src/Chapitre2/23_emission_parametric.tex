...
\section{Effect of electron emission}
  ...
  \begin{figure}[hbt]
    ...
    \caption{Radial profile the electron mobility (left) measured in the \acs{PIC} simulations, and (right) given by \cref{eq-defmobeff}, for different wall emissivities. }
 ...
 Consequently, we observe a few oscillations before the quasi steady-state is reached, as these cases are close to the bifurcation.
   ...
   The oscillations between regimes {\bf I} and {\bf III} defines regimes {\bf II}.
   ...
   The reason for the discrepancy between the expected and the observe threshold value of $\crover$ could be that when $\rate$ increases, the electron power losses increases, hence decreases the electron temperature at equilibrium.
   The evolution of the temperature with  $\rate$ is shown latter in \cref{fig-Tevsproba}.
   We can see that even for very large \ac{SEE} rate, the electron temperature only decreases down to $\Te=30\,\volt$.
   However, the curves of \cref{fig-dphivsTe} still predict for $\Te=30\,\volt$ a threshold value at $\crover=60\,\volt$.
   Consequently, even if $\Te$ does indeed decrease when increasing $\crover$, it remains too large the explain the discrepancy between the observed transition between regime {\bf III} and {\bf II} in \cref{fig-threeregimes} and the curves in \cref{fig-dphivsTe}.
