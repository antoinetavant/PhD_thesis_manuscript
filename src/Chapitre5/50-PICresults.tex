...
\section{Instability in the \acs{2D} radial-azimuthal \acs{PIC} simulations}
  ...
  \Cref{fig-profiles_ne_one} shows the radial profile of the electron and ion densities, as well as the plasma potential, at the end of the simulation. 
  ...
  \Cref{fig-FFT_ex} shows the frequency spectrum of the azimuthal electric field presented in \cref{fig-2DcutEx} computed via \ac{FFT} in the saturated regime between $t > 1.5$ and $7\,\micro\second$.
  ...
  \Cref{fig-turbul} shows the frequency power spectrum computed between $t > 1.5$ and $7\,\micro\second$ in log scale.
  ...
  \paragraph{Ion-wave trapping oscillation\\}
    ...
    \end{figure}
    \inlinenote{Anne: Add unites here}
    ...
    The hypothesis that the low frequency modulation of the amplitude of the azimuthal wave could be validated by varying the ion mass, and observe if $T_{NL}$ varies in $\sqrt{m_i}$.    \inlinenote{Anne: "On peut se demander pourquoi tu ne l'a pas fait"}
