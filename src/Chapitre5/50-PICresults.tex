...
\section{Instability in the \ac{PIC} simulation}
  ...
  Indeed, as we observed the same results when varying the  different physical parameters, there is no need in presenting more results.
  ...
  As the same time of my thesis, other groups presented their simulation results in a similar radial-azimuthal geometries. 
  Their results are discussed at the end of the section, in \cref{subsec-others}.
  ...
  \subsection{Preliminary results of the azimuthal instability }
  ...
  The results presented here emphasize on the azimuthal electric field, but the same characteristic are also observed on the other quantities.
  ...
  We can see in \cref{fig-FFT_ex} a slow decrease of the frequency amplitude from $f={\rm theo}$ to larger frequencies.
  ...
  \begin{figure}[hbtp]
    ...
    \caption{Frequency power spectra, normalized and in log-log scale. Is }
    \label{fig-turbul}
  \end{figure}
  
  \Cref{fig-turbul} shows the frequency power spectra in log scale.
  Is overlaid a fit of the cascade, for frequencies above the maximum amplitude.
  The power law obtained is $\alpha \simeq 4.2$.
  The oscillations observed present a decrease of the power spectra much steeper than expected.
  Even if the mechanism of plasma turbulences differs from the fluid \citep{tsytovich1972}, the value of $\alpha$ is quite large, hence we dismiss this phenomena.
  ...
  Hence, the azimuthal electric field becomore of the type of
  ...
  \paragraph{Ion trapping oscillation\\}
    ...
    But here, we can see that when the criteria \cref{eq-criteriaIT} is fulfilled, the temporal derivative of the growth rate is positive.
    However, a delay $\tau$ after the moment when the criteria is violated ($\tau = 0.40 \pm 0.07 \,\micro\second$ on the four oscillations), the wave abruptly stop rising and decreases instead.
    ...
    A complementary results is presented in \cref{subsec-VDFIAW} by solving the dispersion relations what we present in the next section.
    
    \subsection{Other results from the comunity} \label{subsec-others}
    
    \inlinenote{Je ne suis pas content de cette partie. Comment conclure sur ces results ??}
