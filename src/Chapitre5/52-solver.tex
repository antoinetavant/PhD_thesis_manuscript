...
\section{Solving the kinetic \acs{DR} for general distribution functions}
  ...
  \subsection{Numerical determination of the plasma dielectric function}
   \label{subsec-numepsilon}
  ...
  \subsection{Finding the root of the plasma dielectric function}
...
\subsection{Use of analytic distribution functions} \label{subsec-DRimpact}
  ...
  \inlinenote{The Druyvertaein EVDF is $\epsilon^2$ !!!!!!! Needs to be redone !}
  \begin{equation} \label{eq-druyv}
    f_{D}(v) = \exp \lp C1 \frac{\norm{v}^3}{v_{th}^3}  \rp C2,
  \end{equation}
  where $C1 \simeq 0.63$ and $C2 \sim 0.84$ are two normalizing constants.
