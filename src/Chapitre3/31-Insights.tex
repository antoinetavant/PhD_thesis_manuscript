...
\section{Insights from the PIC simulations}
  ...
  \subsection{Electron distribution function}
    ...
    Using the kinetic information of the PIC simulations, we present in \Cref{fig-EEDF} the mean electron energy probability functions (EEPF) in the case $\crover = 200\,\volt$, foor which the electron emission is of the order of $\rate=0.6$.
    ...
    We see in \cref{fig-EEDF}.{\bf a} that the electron energy distribution function is not Maxwellian.
    ...
    Hence, we can conclude that even if the Maxwellian hypothesis is not respected in the PIC simulations, it does not to explain  the differences observed on $\rate$ in \cref{fig-seeparamesMaxw}.
    ...
    This effect is assessed in the next section.
     ...
     It is possible that when the instability is fully saturated, during the quasi steady-state, the electrons have had sufficient time to isotropize.
   ...
   The significant difference between the value of the average electron temperature in the whole simulation domain, used before in \cref{sec-sheath_validation}, and the electron temperature close to the wall can be responsible for the overestimation of $\rate$ compared to $\ratepic$.
   ...
   To summarise, \cref{fig-rate_pic_wall} shows that when the sheath  is not in the \ac{SCL} regime, the \ac{SEE} rate can be well predicted by \cref{eq-seemaxw} using the electron temperature close to the wall, which is lower than in the center of the domain.
